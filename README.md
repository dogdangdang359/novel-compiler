...
- **世界观图谱**（0.4.4 增强）：开发者模式新增「世界观图谱」面板——`GET /api/worldgraph` 从 canon 以纯函数推导实体关系网络（共现边）、悬念开合状态与时间线流动，前端零依赖 SVG 力导向布局按类型着色（角色蓝/地点灰/物品黄、半径随关联度）、点击节点高亮关联边，右侧悬念台账按 open/planned/resolved 归类。
- **剧情健康度雷达舱**（0.4.4 增强）：「健康度」面板——`GET /api/health` 从 canon 推导六维雷达（悬念回收 / 无过期未收 / 悬念在飞占比 / 角色聚集 / 世界观铺陈 / 时间线覆盖），加权合成 0–100 综合分（优秀/健康/需打磨/需大修，权重见 `src/health.js` 顶部 `HEALTH_WEIGHTS`），附逐章节奏曲线（悬念堆积/活跃实体/新知密度）与分级警戒清单（过期未收/悬空无落点/时间线断档/未回收）。两面板均为本地纯函数推导，不消耗 LLM。
...
- [x] 0.4.4：创作者/开发者双模式（默认创作者，不持久化）——「生成报告」一键流水线（保存→模拟→提取→比对→重规划→编译→自动审阅，失败停在出错步骤）+ 标签中文化（文稿/故事大纲/前情提要，canon 幕后维护不暴露）+ 左上角「文检」设置栏（模式切换）+ 审阅写回后编辑器 canon 模型自动刷新（防旧模型覆盖审阅成果）+ 多 LLM 供应商适配层（DeepSeek/OpenAI/Moonshot/通义千问/智谱/Ollama，见 `LLM 供应商` 节）+ 世界观图谱与剧情健康度雷达舱面板（本地纯函数推导，不耗 LLM，见 IDE 节）
